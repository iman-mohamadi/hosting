...
import { reply_admin_ticket } from "@/actions"
import {
  StatusBadge,
...
import type { AdminPageCopy, AdminTicket } from "@/types/admin"

interface AdminTicketThreadProps {
  ticket: AdminTicket
  copy: AdminPageCopy["tickets"]
  locale: Locale
}
...
export function AdminTicketThread({
  ...
  function handle_submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    start_transition(async () => {
      const result = await reply_admin_ticket(ticket.ticket_id, body)
      if (result.success) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <Link
        href={localizePathname("/admin/tickets", locale)}
        className={cn(
      ...
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          {copy.customer}: {ticket.user_name}{" "}
          <span className="font-mono text-xs">({ticket.user_email})</span>
        </p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-4">
                ...
                >
                  {is_support ? copy.you_support : message.author_name}
                </span>
